...
bool aml_support_dolby_vision()
{
  ...
  if (support_dv == -1)
  {
    ...
    {
      support_dv = (int)((support_info.Get<int>() & 7) == 7);
      CSysfsPath ko_info{"/sys/class/amdolby_vision/ko_info"};
      if (ko_info.Exists())
        CLog::Log(LOGDEBUG, "Amlogic Dolby Vision info: {}", ko_info.Get<std::string>().c_str());
    }
  }
...
int aml_axis_value(AML_DISPLAY_AXIS_PARAM param)
{
  ...
  if (display_axis.Exists())
    axis = display_axis.Get<std::string>();
...
bool aml_get_native_resolution(RESOLUTION_INFO *res)
{
  ...
  if (display_mode.Exists())
    mode = display_mode.Get<std::string>();
...
bool aml_probe_resolutions(std::vector<RESOLUTION_INFO> &resolutions)
{
  ...
  if (!user_dcapfile.Exists())
  {
    ...
    if (dcapfile.Exists())
      valstr = dcapfile.Get<std::string>();
    else
      return false;
  ...
  bool force_mode_switch)
{
  ...
  CSysfsPath display_mode{"/sys/class/display/mode"};
  if (display_mode.Exists())
    cur_mode = display_mode.Get<std::string>();
